...
import com.zjlp.face.ums.util.LcUmsUtil;
import com.zjlp.face.ums.util.PropertiesUtil;
import com.zjlp.face.ums.util.UmsUtil;
@Service("umsService")
public class LcUmsServiceImpl implements UmsService {
	...
	public boolean send(String[] messages, String modelId, String mobile) {
		try{
			...
			boolean bool = LcUmsUtil.send(model, mobile);
	...
	public String sendByJson(String[] messages, String modelId, String mobile) {
		try{
			...
			_logger.info(new StringBuffer("短信内容：").append(model).toString());
			String json = LcUmsUtil.sendByJson(model, mobile,UmsService.MEDIA_TYPE_SMS);
	...
	@Override
	public String sendByJson4Audio(String[] messages,String mobile) {
		try{
			_logger.info("{\"messages\":\""+JSONArray.fromObject(messages).toString()+
			...
			String json = LcUmsUtil.sendByJson(model, mobile,UmsService.MEDIA_TYPE_AUDIO);
